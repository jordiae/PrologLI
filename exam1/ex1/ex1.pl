...
% nurseType-N-Type     "nurse N has type T"        (Type=1: ex1 WWRWWRWW, Type=2: ex1 WWRWWRRWW, Type=3: ex1 WWRRWWRWW, Type=4: ex1 WWRRWWRRWW)

%%%%%%%%% Some helpful definitions to make the code cleaner:

nurse(N):- nurseIDandBlocking(N,_,_).
hour(H):- between(0,23,H).
type(T):- between(1,4,T). % 4 types
needed(H,N):- needs(L), member(H-N,L).

workingHourForTypeAndStartH(1,StartH,H):- between(0,1,I), H is (StartH+I) mod 24. % Type=1: ex1 WWRWWRWW
...
workingHourForTypeAndStartH(2,StartH,H):- between(0,1,I), H is (StartH+I) mod 24. % Type=2: ex1 WWRWWRRWW
workingHourForTypeAndStartH(2,StartH,H):- between(3,4,I), H is (StartH+I) mod 24.
workingHourForTypeAndStartH(2,StartH,H):- between(7,8,I), H is (StartH+I) mod 24.

workingHourForTypeAndStartH(3,StartH,H):- between(0,1,I), H is (StartH+I) mod 24. % Type=3: ex1 WWRRWWRWW
workingHourForTypeAndStartH(3,StartH,H):- between(4,5,I), H is (StartH+I) mod 24.
workingHourForTypeAndStartH(3,StartH,H):- between(7,8,I), H is (StartH+I) mod 24.

workingHourForTypeAndStartH(4,StartH,H):- between(0,1,I), H is (StartH+I) mod 24. % Type=4: ex1 WWRRWWRRWW
workingHourForTypeAndStartH(4,StartH,H):- between(4,5,I), H is (StartH+I) mod 24.
workingHourForTypeAndStartH(4,StartH,H):- between(8,9,I), H is (StartH+I) mod 24.
...
writeClauses:- 
	eachNurseExactlyOneStart,
	satisfyDemand,
	blockedHours,
	followPatterns,
	followPatternsNo,
	eachNurseExactlyOneType.

eachNurseExactlyOneStart:-
	nurse(N),
	findall(startsNH-N-H, hour(H), C),
	exactly(1,C),
	fail.
eachNurseExactlyOneStart.
...
eachNurseExactlyOneType:-
	nurse(N),
	findall(nurseType-N-T, type(T), C),
	exactly(1,C),
	fail.
eachNurseExactlyOneType.
...
followPatternsNo:-
	...
	hour(StartH),
	type(T),
	findall(H,workingHourForTypeAndStartH(T,StartH,H),R),
	hour(H),
	not(member(H,R)),
	writeClause([\+startsNH-N-StartH, \+nurseType-N-T,\+worksNH-N-H]),
	fail.
