...
% a partir d aqui no verificat profe
satisfyDemand:-
	hour(H),
	needed(H,Nnurses),
...
blockedHours:-
    ...
    % Funcionava pero no tenia sentit, canviem les dues seguents linies pel writeClause
	%findall(worksNH-N-H,hour(H),C),
	%exactly(0,C),
...
followPatterns:-
  ...
  % Per tota infermera:
  % si el seu tipus es 1, aleshores WWWRWWW. Si es 2, WWWRRWWW
  % necessitem saber el tipus i startH
   nurse(N),
   %findall(startsNH-N-H,hour(H),LS),
   %findall(nurseType-N-T,type(T),LT),
   %member(N-StartH,LS),
   %member(N-Type,LT),
   type(Type),
   hour(StartH),
   findall(worksNH-N-H,workingHourForTypeAndStartH(Type,StartH,H),C),
   exactly(6,C),
...
followPatterns2:-
	...
	hour(StartH),
	type(T),
	findall(worksNH-N-H,workingHourForTypeAndStartH(T,StartH,H),C),
	negate(startsNH-N-H,L1),
	negate(nurseType-N-T,L2),
	expressAnd([L1,L2],C).
...
followPatterns3:-
	...
	writeClause([\+startsNH-N-StartH,\+nurseTypeNH-N-T,worksNH-N-H]),
	fail.
...
followPatterns3:-
	...
	hour(StartH),
    type(T),
	notWorkingHourForTypeAndStartH(T,StartH,HN),
	writeClause([\+startsNH-N-StartH,\+nurseTypeNH-N-T,\+worksNH-N-HN]),
	fail.
followPatterns3.

notWorkingHourForTypeAndStartH(T,StartH,HN):-
	hour(HN),\+workingHourForTypeAndStartH(T,StartH,HN).
